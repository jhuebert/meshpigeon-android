package app.meshhop.feature.contacts
...
fun ContactsScreen(contacts: List<Contact> = emptyList()) {
    ...
    Column(modifier = Modifier.fillMaxSize()) {
        ...
        val shown = when (tab) {
            ...
            1 -> contacts.filter { it.source == app.meshhop.domain.ContactSource.ADVERT && !it.isBlocked }
...
fun ContactRow(contact: Contact) {
    Card(modifier = Modifier.fillMaxWidth()) {
        Row(
            modifier = Modifier.padding(MeshHopSpacing.md),
