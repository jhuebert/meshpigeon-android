package app.meshhop.transport.android
...
import app.meshhop.transport.FrameStream
import app.meshhop.transport.RadioAdapter
import app.meshhop.transport.RadioFrame
import app.meshhop.transport.RadioLink
import app.meshhop.transport.RadioLinkState
import app.meshhop.transport.RadioTarget
import app.meshhop.transport.TransportFrameCodec
 ...
 *
 * Nordic UART Service (must match meshhop-firmware/src/transports.h):
 */
object NordicUart {
...
class BleRadioAdapter(
        ...
        override fun onServicesDiscovered(g: BluetoothGatt, status: Int) {
            val svc = g.getService(NordicUart.SERVICE) ?: run {
                state.value = RadioLinkState(
                    ...
                    error = "radio has no MeshHop UART service",
                )
                return
            }
    ...
    override fun scan(): Flow<RadioTarget> = callbackFlow {
        ...
        scope.launch(Dispatchers.Main) {
            // scan for the NUS service UUID advertised by MeshHop radios
                ...
                override fun onScanResult(callbackType: Int, result: android.bluetooth.le.ScanResult) {
                    trySend(
                        RadioTarget(
                            ...
                            name = result.scanRecord?.deviceName ?: "MeshHop radio",
